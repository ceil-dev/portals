...
export type PortalSupplier<TPayload = unknown, TReturn = any> = Supplier<
  PortalSupplierData<TPayload>,
  TReturn,
  CoreSuppliers
>;
...
type LocalEnterPayload = { sender: string } & Guest;
...
type CallPayload = { key: string; value: Serializable[]; sender: string };
type ResolvePayload = { key: string; err?: string; value?: Serializable };
...
type SendPayload = {
  recipient: string;
  payload: { sync: number; queue?: QueueOutEntry[] };
};

type EtherSendPayload = { recipient: string; payload?: unknown };

type RemotePayload = { recipient: string } & Record<string, Serializable>;
...
type RemoteEnterPayload = { id: string };

type ReceiveReturn = { sender: string };

export type InitSupplier = Supplier<
  { id: string; env: MicroEnv },
  PortalMethod,
  CoreSuppliers & { debug: DebugSupplier }
>;
export type SendSupplier = PortalSupplier<SendPayload, Promise<unknown>>;
export type ReceiveSupplier = PortalSupplier<
  unknown,
  Promise<ReceiveReturn | undefined>
>;
type QueueOperation = 'in' | 'out' | 'process' | 'dispatch' | 'reset';
export type QueueSupplier = PortalSupplier<
  [op: QueueOperation, payload?: any, settings?: any],
  Promise<{ result?: unknown } | undefined | void>
>;
...
export type EtherRestartSupplier = PortalSupplier<{ recipient: string }>;
export type LocalEnterSupplier = PortalSupplier<
  LocalEnterPayload,
  EnterSupplierReturn
>;
export type LocalRestartSupplier = PortalSupplier<{ sender: string }>;
...
export type RemoteEnterSupplier = PortalSupplier<
  RemoteEnterPayload,
  EnterSupplierReturn
>;
...
export type PackagerPackSupplier = PortalSupplier<
  Serializable,
...
export type PackagerUnpackSupplier = PortalSupplier<
  unknown,
  UnpackSupplierReturn | Promise<UnpackSupplierReturn>
>;
export type CryptoSupplier = PortalSupplier<void, { randomUUID: () => string }>;
...
export type Middleware<T extends Suppliers = CoreSuppliers> =
  | ExtendedSuppliers<T>
  | ((s: T) => ExtendedSuppliers<T>);

export type EtherMiddleware = Middleware<
  Required<
    Pick<CoreSuppliers, 'ether.attach' | 'ether.send' | 'ether.detach'>
  > &
    Pick<CoreSuppliers, 'receive'>
>;

/////// the only way it was possible to make the inference worked /////////
// const portal = <K extends keyof CoreSuppliers>(
//   key: K | string,
//   payload: CoreSuppliers[K] extends PortalSupplier<infer P> ? P : any
// ) => {
//   return {} as any;
// };
// // export type PortalMethod = typeof portal;
export type PortalMethod = <
  T extends keyof CoreSuppliers = keyof CoreSuppliers,
>(
  key: T,
  ...rest: T extends keyof CoreSuppliers
    ? CoreSuppliers[T] extends PortalSupplier<infer P1>
      ? P1 extends undefined
        ? [payload?: undefined]
        : [payload: P1]
      : [unknown]
    : [unknown]
) => T extends keyof CoreSuppliers
  ? CoreSuppliers[T] extends PortalSupplier<any, infer P2>
    ? P2
    : unknown
  : unknown;
///////////////////////////////////////////////////////////////////////////
